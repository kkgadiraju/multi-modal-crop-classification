...
def crop_generator(input_path, batch_size=32, mode="train", num_classes =6, epsilon = 0, resize_params = (224, 224), do_shuffle=True):	
    ...
    while True:
        ...
        imgs, ts, labels = preprocess_batches(image_batch= curr_batch, class_names = [0,1,2,3,4,5], num_classes = num_classes, epsilon = epsilon, resize_width_and_height=resize_params, mode=mode) 
        imgs = np.array(imgs)
        #print(imgs.shape)
        ts = np.array(ts)
        #print(ts.shape)
