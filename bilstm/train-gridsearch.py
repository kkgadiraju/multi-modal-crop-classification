...
from tensorflow.python.keras.layers import Input, LSTM, Dense, TimeDistributed, concatenate, GlobalAveragePooling2D, Dropout, Bidirectional
...
for idx, row in grid_df.iterrows(): 
    if row['timestamp'] == 'None':
        ...
        if time_passed_since_start_in_hours >= 21.:
            #print(f'Not enough time for lr = {learning_rate}, dropout = {dropout_rate}')
            ...
            # add an LSTM layer
            lstm_1 = Bidirectional(LSTM(hidden_units, input_shape=(23,1), dropout=dropout_rate))(temporal_input_layer)
