...
grid_file = f'./{config[parser_args.task]}_best_multi.csv'

if os.path.exists(grid_file):
    grid_df = pd.read_csv(grid_file)

else:
    grid_df = pd.DataFrame({'lr':[learning_rate], 'd':[dropout_rate], 'timestamp':['None']})
    grid_df = pd.concat([grid_df]*10, ignore_index=True)        
...
for idx, row in grid_df.iterrows(): 
    if row['timestamp'] == 'None':
        ...
        print(f"Processing row = {idx},  lr = {learning_rate}, dropout = {dropout_rate}")
